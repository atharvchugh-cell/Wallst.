...
**How it works.** Each processed date `T`, every sleeve is **re-derived by replaying the ordinary backtest engine from the account's inception up to `T`** — the same strategy / market-view / engine machinery the backtest and `--strategy portfolio` modes use, so paper behavior cannot silently diverge from backtest behavior. Because that replay only ever reads data on or before `T`, there is **no look-ahead** and re-processing a date is **idempotent** (same inputs → same ledger; a date already processed is a no-op). Signals generated on date `T` become **pending orders** that fill at the **next** session's close (the existing one-trading-day signal-to-fill lag — no same-close execution); an order whose fill price is unavailable is reported **stale**, never silently filled.

**Static, independent sleeves.** The account splits capital once at init — **60% momentum / 35% sector_rotation / 5% regime_switch** (`$9,000 / $5,250 / $750` of `$15,000`) — and each sleeve runs fully independently: **separate cash, separate positions, no shared cash pool, no cross-sleeve borrowing, no rebalancing between sleeves.** Weights then drift with performance. Every processed session runs an internal **reconciliation** (portfolio equity = sum of sleeve equity; sleeve equity = cash + market value of positions; fills reconcile with cash/position/cost changes; allocations sum to capital) and **fails closed** — leaving the prior state intact — if it does not balance.

**State safety.** State is split into `paper_config.json` (immutable account identity — created-at timestamp, starting capital, weights, frozen universe snapshot + version, strategy params, git SHA, schema version) and `paper_state.json` (the evolving ledger). Both are written **atomically** (temp file + fsync + rename), a **file lock** prevents concurrent runs from corrupting state, the schema is **validated on load**, and corrupt/incomplete state **fails closed and is never overwritten**. `--paper-reset` **backs up before removing** and requires the exact `--confirm-paper-reset` flag.

**Artifacts** (in the state dir, and via `--paper-export`): `paper_status.txt`, `paper_orders.csv`, `paper_trades.csv`, `paper_positions.csv`, `paper_equity.csv`, `paper_sleeves.csv`, `paper_run_log.csv`, `paper_state.json`, `paper_config.json`. Every report carries the notices **PAPER TRADING ONLY / NO REAL ORDERS WERE SENT / NOT FINANCIAL ADVICE / NO BROKERAGE CONNECTION EXISTS / RESULTS MAY DIFFER FROM LIVE EXECUTION**.
