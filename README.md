...
78 tests across indicator math, the look-ahead-prevention accessor, both strategies' decision logic (including explicit "does the decision change if I mutate any future date" checks), the engine's cash/lot accounting (including net-of-cost P&L and trading-day holding periods), cost/turnover metrics, the data/caching layer (including the yfinance end-date-inclusivity fix and stale-cache-fallback warnings), and the CLI's minimum-usable-universe guard. CI runs this suite on every push via GitHub Actions.
