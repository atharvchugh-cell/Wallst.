...
### `--strategy paper`: forward paper-trading ledger for the fixed 60/35/5 portfolio

Everything above is a *batch* backtest. `--strategy paper` is different: it maintains a **persistent, forward paper-trading account** that processes finalized market sessions **one at a time**, generating simulated orders using only information available as of each processed date and writing a reloadable ledger under `paper_state/`. It is **research / paper trading only** — it never connects to Robinhood or any broker, never transmits an order, and holds no credentials.

```bash
# 1. Initialize a $15,000 account (defaults to a today-dated inception; pass
#    --paper-start to backdate it for a deterministic historical simulation):
python backtest.py --strategy paper --paper-init --capital 15000 --paper-start 2023-01-01

# 2. Process forward, one finalized session at a time:
python backtest.py --strategy paper --paper-run                       # next unprocessed session
python backtest.py --strategy paper --paper-date 2023-03-31           # forward THROUGH a date

# 3. Inspect / export the ledger:
python backtest.py --strategy paper --paper-status                    # account snapshot
python backtest.py --strategy paper --paper-orders                    # pending (and stale) orders
python backtest.py --strategy paper --paper-trades                    # completed paper fills
python backtest.py --strategy paper --paper-reconcile                 # re-verify accounting invariants
python backtest.py --strategy paper --paper-export --output-dir output  # timestamped artifact copy

# 4. Reset safely (backs up first; the exact confirm flag is required):
python backtest.py --strategy paper --paper-reset --confirm-paper-reset
```

**How it works — a true incremental ledger, not a full-history replay.** The authoritative store is a per-account **SQLite database** (`paper_state/paper_ledger.db`, schema in `src/paper_db.py`): append-only `signals` / `orders` / `fills` / `realized_trades` / `equity_history` / `sleeve_equity_history` / `run_log` tables, plus materialized (but always independently reconstructible from the fills ledger) `sleeves` / `positions` tables. Each `--paper-run` / `--paper-date` call processes **one finalized session at a time**, strictly incrementally, per sleeve, inside **one SQLite transaction**:
1. **fill (or mark stale)** only this sleeve's already-persisted **pending orders** scheduled for this session, using the finalized closing price — the one-trading-day signal-to-fill lag, no same-close execution, never a silent fill when the price is unavailable;
2. update cash/positions **incrementally** (average-cost accounting, reusing `engine._execute_events` — the exact buy/sell/cash-scaling code the backtest engine uses — applied to the persisted state, not a fresh walk);
3. mark every current holding to market and record one equity row per sleeve, then one portfolio-level row;
4. generate this session's **new** signals by replaying the strategy from inception through today (reusing `tournament.prepare_stock_plan`/`prepare_sector_plan` + `engine.run_backtest`, so signal generation can never diverge from backtest logic) — but that replay's *accounting* (cash, positions, historical fills) is **entirely discarded**; only brand-new signals (`signal_date == today`) are ever persisted, as new rows;
5. **independently reconcile** the ledger from the immutable fills table (never from the materialized `sleeves`/`positions` tables) and **commit the whole session only if every invariant passes** — otherwise roll back everything and leave prior state byte-for-byte intact.

Because there is no "replay from scratch and overwrite" step, **market-data revisions, later strategy-code edits, or config changes can never rewrite an already-committed session** — proven by a regression test that mutates historical prices *after* a fill and confirms every previously persisted signal/order/fill/cost/cash/equity row is unchanged.

**Frozen strategy identity.** A deterministic **config fingerprint** (SHA-256 over the fixed weights, strategy params, cost/fractional-share assumptions, frozen universe, the session-processing algorithm version, and each in-use strategy's own source code) is computed at `--paper-init` and checked before every advance. If the runtime code or config has drifted from what the account was initialized with, **advancing is refused** (fail closed, no silent recompute under changed code); every read-only command (`--paper-status`/`--paper-orders`/`--paper-trades`/`--paper-export`/`--paper-reconcile`) keeps working regardless.

**Static, independent sleeves.** The account splits capital once at init — **60% momentum / 35% sector_rotation / 5% regime_switch** (`$9,000 / $5,250 / $750` of `$15,000`) — and each sleeve runs fully independently: **separate cash, separate positions, no shared cash pool, no cross-sleeve borrowing, no rebalancing between sleeves.** Weights then drift with performance.

**Stable IDs, provenance, and holiday-aware scheduling.** Every signal/order/fill has a **stable surrogate ID** (never derived solely from ticker/side/date — supporting multiple same-ticker fills on one date without collision), with foreign-key linkage `order → signal` and `fill → order`; a duplicate signal is rejected by a **database constraint**. Every fill records its full decision provenance: signal date, data cutoff, sizing equity/price, scheduled vs. actual fill date, fill price, price semantics (adjusted close), data source, and retrieval timestamp. A pending order's scheduled fill date comes from `src/nyse_calendar.py`, a fixed **holiday-aware NYSE session calendar** (weekends + MLK Day, Presidents Day, Good Friday, Memorial Day, Juneteenth, July 4th, Labor Day, Thanksgiving, Christmas — with standard Saturday/Sunday-observed shifts), not a naive weekend-only projection.

**Validation, reconciliation, and recovery.** Before advancing, the persisted ledger is independently re-validated (types/finite values, no duplicate IDs, strictly ordered sessions, foreign-key integrity, valid order-status transitions, no negative cash beyond tolerance, position quantities and cash reconciling from the fills ledger, portfolio equity = sum of sleeve equity, fixed initial allocations summing exactly to starting capital, pending orders never counted as fills or holdings) — **any semantic inconsistency fails closed without silently repairing it**. `--paper-reconcile` runs the identical check on demand and additionally detects tampering: deleted fill rows, altered fill values (via a per-row hash), broken foreign-key links, duplicate equity dates, and invalid status transitions. Before each session's transaction begins, a **recoverable snapshot** of the database is taken (`paper_state/backups/`); a mid-session exception rolls back **every** table write for that session (proven by a test that injects a failure partway through and confirms no partial fill/cost/position/equity/run-log row survives). A **file lock** blocks concurrent runs from corrupting state.

**Artifacts** (in the state dir, and via `--paper-export`, which also includes the authoritative `paper_ledger.db`): `paper_status.txt`, `paper_orders.csv`, `paper_trades.csv`, `paper_positions.csv`, `paper_equity.csv`, `paper_sleeves.csv`, `paper_run_log.csv`, `paper_state.json`, `paper_config.json` — the last two are **non-authoritative exported views** of the SQLite ledger (safe to regenerate any time), not the source of truth. Every run summary reports the paper date, data cutoff, new signals, pending orders created, fills, rejected/stale orders, current positions, cash/value by sleeve, total equity, cumulative/daily return, transaction costs, reconciliation status, and any warnings/deviations. Every report carries the notices **PAPER TRADING ONLY / NO REAL ORDERS WERE SENT / NOT FINANCIAL ADVICE / NO BROKERAGE CONNECTION EXISTS / RESULTS MAY DIFFER FROM LIVE EXECUTION**.

**Same disclosures as every other mode.** Paper trading does **not** fix survivorship bias — it uses the same current-snapshot universe as every backtest — and results may differ from live execution (no intraday assumptions, no bid/ask spread or market impact, pre-tax). It is a research/paper-trading tool, not a broker, and never places a live order.
